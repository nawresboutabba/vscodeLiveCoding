...
class RecordingStatus {
    ...
    async save() {
        ...
        if (vscode.workspace.workspaceFolders !== undefined) {
            x = 1;
            const answer = await vscode.window.showInformationMessage(vscode.workspace.workspaceFolders[0].uri.path.substring(1, vscode.workspace.workspaceFolders[0].uri.path.length));
            ...
            await exec('cd ' + vscode.workspace.workspaceFolders[0].uri.path.substring(1, vscode.workspace.workspaceFolders[0].uri.path.length) + ' & echo savefile.json > .gitignore', (err, stdout, stderr) => {
                // handle err, stdout & stderr
            });
            ...
            while (x == 1) {
                ...
                exec('cd ' + vscode.workspace.workspaceFolders[0].uri.path.substring(1, vscode.workspace.workspaceFolders[0].uri.path.length) + ' & git diff --exit-code', (err, stdout, stderr) => {
                    // handle err, stdout & stderr
                    changed = stdout;
                });
                ...
                if (changed !== '') {
                    ...
                    await exec('cd ' + vscode.workspace.workspaceFolders[0].uri.path.substring(1, vscode.workspace.workspaceFolders[0].uri.path.length) + ' & git rev-parse HEAD', (err, stdout, stderr) => gitid = stdout.substring(0, stdout.length - 1));
                    t1 = performance.now();
            ...
            setTimeout(function () {
                if (vscode.workspace.workspaceFolders !== undefined) {
                    exec('cd ' + vscode.workspace.workspaceFolders[0].uri.path.substring(1, vscode.workspace.workspaceFolders[0].uri.path.length) + ' & git add --all & git commit -m verFinale & git rev-parse HEAD', (err, stdout, stderr) => {
    ...
    async countDown(seconds) {
        if (seconds === undefined) {
            const defs = await config_1.Config.getRecordingDefaults();
